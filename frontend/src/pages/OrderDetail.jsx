  ...
  return (
    <Container className="mt-4 mb-5">
      <div className="d-flex justify-content-between align-items-center mb-4">
        ...
        <Button variant="outline-secondary" onClick={() => navigate(isAdmin ? '/admin' : '/dashboard')}>⬅ Quay lại</Button>
      </div>
      ...
      {/* 2. THANH TIẾN TRÌNH ĐÃ ĐƯỢC SỬA MÀU CHUẨN XÁC */}
      <Card className="shadow-sm border-0 mb-4 p-4 text-center">
        ...
        <div className="d-flex justify-content-between align-items-center position-relative mx-auto" style={{ maxWidth: '800px' }}>
            <div style={{ position: 'absolute', top: '50%', left: '10%', right: '10%', height: '4px', background: '#e9ecef', zIndex: 0 }}></div>
            
            <Badge bg={['arrived_pickup', 'picking_up', 'delivering', 'completed'].includes(orderDetails.order.status) ? "success" : "primary"} className="p-3 rounded-circle shadow-sm position-relative z-index-1" style={{ width: '50px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>1</Badge>
            <Badge bg={['delivering', 'completed'].includes(orderDetails.order.status) ? "success" : ['arrived_pickup', 'picking_up'].includes(orderDetails.order.status) ? "primary" : "light"} text={['arrived_pickup', 'picking_up', 'delivering', 'completed'].includes(orderDetails.order.status) ? "white" : "dark"} className="p-3 rounded-circle shadow-sm position-relative z-index-1 border" style={{ width: '50px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>2</Badge>
            <Badge bg={orderDetails.order.status === 'completed' ? "success" : orderDetails.order.status === 'delivering' ? "primary" : "light"} text={['delivering', 'completed'].includes(orderDetails.order.status) ? "white" : "dark"} className="p-3 rounded-circle shadow-sm position-relative z-index-1 border" style={{ width: '50px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>3</Badge>
            <Badge bg={orderDetails.order.status === 'completed' ? "success" : "light"} text={orderDetails.order.status === 'completed' ? "white" : "dark"} className="p-3 rounded-circle shadow-sm position-relative z-index-1 border" style={{ width: '50px', height: '50px', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.2rem' }}>4</Badge>
        </div>
        <div className="d-flex justify-content-between mx-auto mt-2 text-muted fw-bold" style={{ maxWidth: '840px', fontSize: '13px' }}>
      ...
      {/* 3. NẾU LÀ ADMIN, CỘT TRÁI SẼ RA GIỮA (justify-content-center) */}
      <Row className={isAdmin ? "justify-content-center" : ""}>
        <Col lg={isAdmin ? 8 : 7} className="mb-4">
          ...
          {/* 4. HIỂN THỊ LẠI LOG CHAT CHO ADMIN Ở DƯỚI CÙNG (CHỈ ĐỌC) */}
          {isAdmin && (
            <Card className="shadow-sm border-0 mt-4 mb-4 border-info border-start border-4">
              <Card.Body className="bg-light">
                <h5 className="fw-bold text-info border-bottom pb-2 mb-3">💬 Nhật ký Chat (Khách ↔ Tài xế)</h5>
                {messages.length === 0 ? (
                  ...
                  <div style={{ maxHeight: '350px', overflowY: 'auto' }} className="pe-2">
                    {messages.map(msg => {
                      ...
                      if (isSystem) return <div key={msg.id} className="text-center my-2"><Badge bg="danger" className="text-wrap shadow-sm p-2">{msg.content}</Badge></div>;
                      
                      const isDriver = msg.sender_id === orderDetails?.order?.driver_id;
                      return (
                        <div key={msg.id} className="mb-2 bg-white p-3 rounded-3 border shadow-sm">
          ...
          {orderDetails.order.rating && (
            <Card className="shadow-sm border-0 mt-4 mb-4 border-warning border-start border-4">
              <Card.Body className="bg-light">
                <h5 className="text-warning mb-2 fw-bold">⭐ Đánh giá từ khách hàng:</h5>
        ...
        {/* 5. ẨN KHUNG CHAT TƯƠNG TÁC NẾU LÀ ADMIN */}
        {!isAdmin && (
          <Col lg={5}>
      ...
      <Modal show={showPodModal} onHide={() => setShowPodModal(false)} centered backdrop="static">
        <Modal.Header closeButton className="bg-success text-white">
          <Modal.Title>📸 {podActionType === 'pickup' ? 'CHỤP ẢNH LẤY HÀNG' : 'CHỤP ẢNH GIAO HÀNG'}</Modal.Title>
        </Modal.Header>
        <Form onSubmit={handleCompleteWithPod}>
          <Modal.Body className="text-center p-4">
            <Alert variant="warning" className="fw-bold mb-4">Vui lòng chụp rõ mã vận đơn và tình trạng hàng hóa để đối chiếu!</Alert>
